...
from app.fill import fill, send as send_reply
...
def fill_reply(text, replace=False):
    ...
    fill(state["hwnd"], state["area"], text, replace=replace)


def auto_send_reply(title, revision):
    """倒计时结束后的最后门禁：会话、版本、微信前台状态任一变化都不发。"""
    if state["hwnd"] is None or state["area"] is None:
        raise RuntimeError("微信输入区已不可用")
    if state["chat"] != title:
        raise RuntimeError("微信已切换到其他会话")
    if chat_of(title)["rev"] != revision:
        raise RuntimeError("倒计时期间收到了新消息")
...
def start_analyze(title, msgs):
    if not settings.has_key():
        ...
        return
    if settings.judge_provider() == "typesafe" and not settings.has_typesafe_key():
        ov.set_status("请先在设置中填写 TypeSafe 官方 API 密钥", "warning")
        return
    state["busy"] = True
    ov.set_busy(True)
                ...
                if title == ov.current_chat():
                    ...
                    if settings.auto_send() and r.get("best_reply"):
                        ov.begin_auto_send(
                            ...
                            on_fill=lambda text: fill_reply(text, replace=True),
                        )
                else:
                    ov.set_busy(False)
            else:
